...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (masterAddress == nullptr) {
            ...
            gamePeer = std::make_unique<GamePeer>(window.getWidth(), window.getHeight(), 60, 15, renderer, masterPort);
        } else {
            INFO("I am a normal peer. Trying to connect to master at {0}:{1}.", masterAddress, masterPort);
            gamePeer = std::make_unique<GamePeer>(window.getWidth(), window.getHeight(), 60, 15, renderer, masterAddress, masterPort);
        }
