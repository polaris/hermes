...
int main(int argc, char** argv) {
    ...
    int c = 0;
    while ((c = getopt(argc, argv, "m:p:l:h")) != -1) {
        switch (c) {
        case 'm':
    ...
    try {
        ...
        if (masterAddress == nullptr) {
            ...
            gamePeer = std::make_unique<GamePeer>(window.getWidth(), window.getHeight(), 60, 15, emulatedLatency, renderer, masterPort);
        } else {
            INFO("I am a normal peer. Trying to connect to master at {0}:{1}.", masterAddress, masterPort);
            gamePeer = std::make_unique<GamePeer>(window.getWidth(), window.getHeight(), 60, 15, emulatedLatency, renderer, masterAddress, masterPort);
        }
